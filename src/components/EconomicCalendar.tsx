...
import { Activity, Layers, Zap, Loader2, RefreshCcw, TrendingUp, TrendingDown, Minus, Info } from 'lucide-react';
...
export function EconomicCalendar() {
  ...
  const SentimentIcon = ({ sentiment }: { sentiment?: string }) => {
    switch (sentiment?.toLowerCase()) {
      case 'trade':
      case 'bullish': 
        return <TrendingUp className="w-4 h-4 text-emerald-400" title="Trade" />;
      case 'avoid':
      case 'bearish': 
        return <TrendingDown className="w-4 h-4 text-rose-400" title="Avoid" />;
      default: 
        return <Minus className="w-4 h-4 text-slate-400" title="Neutral" />;
    }
  };

  if (loading && Object.keys(weeklyData).length === 0) {
    return (
      <div className="flex flex-col h-full bg-[#050508] text-foreground font-body">
  ...
  return (
    <div className="flex flex-col h-full bg-[#050508] text-foreground font-body overflow-hidden">
      <main className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-6 gap-6 bg-[#08090d] overflow-y-auto">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-white/5 hover:bg-transparent">
                    <TableHead className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Time</TableHead>
                    <TableHead className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Currency</TableHead>
                    <TableHead className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Event</TableHead>
                    <TableHead className="text-slate-400 font-bold uppercase text-[10px] tracking-widest text-center">Impact</TableHead>
                    <TableHead className="text-slate-400 font-bold uppercase text-[10px] tracking-widest text-right">Actual</TableHead>
                    <TableHead className="text-slate-400 font-bold uppercase text-[10px] tracking-widest text-right">Forecast</TableHead>
                    <TableHead className="text-slate-400 font-bold uppercase text-[10px] tracking-widest text-right">Previous</TableHead>
                ...
                <TableBody>
                  {filteredEvents.map((event) => (
                    <TableRow key={event.id} className="border-white/5 hover:bg-white/5 transition-colors">
                      <TableCell className="font-medium py-4 text-xs">{event.time}</TableCell>
                      <TableCell className="font-bold text-xs">{event.currency}</TableCell>
                      <TableCell className="text-xs">{event.event}</TableCell>
                      <TableCell className="text-center">
                        <ImpactBadge impact={event.impact} />
                      </TableCell>
                      <TableCell className="text-right font-mono text-xs">{event.actual || '-'}</TableCell>
                      <TableCell className="text-right font-mono text-xs text-white/60">{event.forecast || '-'}</TableCell>
                      <TableCell className="text-right font-mono text-xs text-white/40">{event.previous || '-'}</TableCell>
                      <TableCell className="text-center">
                        <SentimentIcon sentiment={event.impact === 'High' ? 'Avoid' : 'Trade'} />
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
