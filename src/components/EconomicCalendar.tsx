...
import { getWeeklyMarketOverviewFlow } from '@/ai/flows/ai-weekly-market-overview-flow';
...
export const EconomicCalendar = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Trigger AI Analysis for the week
      const allEvents = Object.values(data).flat();
      if (allEvents.length > 0) {
        const analysis = await getWeeklyMarketOverviewFlow({ 
          economicEvents: allEvents.slice(0, 50) 
        });
  ...
  const selectedDayEvents = useMemo(() => {
    if (!selectedDate || !weeklyData[selectedDate]) return [];
    
    return weeklyData[selectedDate].map(event => {
      const detail = aiAnalysis?.eventsDetail?.find((d: any) => d.id === event.id);
      return {
        ...event,
        sentiment: detail?.sentiment || 'Neutral',
        impact_percentage: detail?.impact_percentage || (event.impact === 'High' ? 85 : event.impact === 'Medium' ? 50 : 20)
      };
    });
  }, [selectedDate, weeklyData, aiAnalysis]);
  ...
  const SentimentIcon = ({ sentiment }: { sentiment: string }) => {
    switch (sentiment) {
      ...
      case 'Bearish': return <TrendingDown className="w-4 h-4 text-rose-400" />;
  ...
  if (loading && Object.keys(weeklyData).length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-[#050508]">
        ...
        <p className="mt-4 text-sm font-medium text-primary">Synchronizing Global Markets...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      <AISidebar 
        ...
        weeklyEvents={Object.values(weeklyData).flat()}
      />
                  ...
                  <div className="flex-1 min-w-0 pr-4">
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      ...
                      <div className="flex items-center gap-1">
                         ...
                         <span className="text-[10px] font-mono text-primary">{event.impact_percentage}%</span>
                      </div>
                      <div className="flex items-center gap-1">
                         <span className="text-[10px] font-bold text-muted-foreground uppercase">Bias:</span>
                         <SentimentIcon sentiment={event.sentiment || 'Neutral'} />
                      </div>
                    </div>
                  </div>
